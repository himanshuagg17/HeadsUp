...
userRouter.post("/register", async (req, res) => {
  const { name, email, password, username,status,permission } = req.body;
  try {
    bcrypt.hash(password, 5, async (err, hash) => {
      if (err) res.send(err.message);
      else {
        // const userData=req.body;
        const user = new UserModel({ name, email, password: hash,username,status,permission });
        await user.save();
  ...
  userRouter.post("/login", async (req, res) => {
    ...
    try {
      ...
      if (user.length > 0) {
        //we are comparing if the password and the bcrypted passwords are matching or not.
        bcrypt.compare(password, user[0].password, (err, result) => {
          if (result) {
            ...
            const token = jwt.sign({ userID: user[0]._id }, "HeadsUp");
